...
int32_t sys_read(int32_t fd, void* buf, uint32_t count) {
	...
	if (fd < 0 || fd == stdout_no || fd == stderr_no) {
		...
		uint32_t fd_ = fd_local2global(fd);
		ret = file_read(&file_table[fd_], buf, count);
	}
	return ret;
}
...
/* 获取目录child_inode_nr的父目录inode编号 */
static uint32_t get_parent_dir_inode_nr(uint32_t child_inode_nr, void* io_buf) {
	struct inode* child_dir_inode = inode_open(cur_part, child_inode_nr);
	/* 目录中的目录项..包括父目录inode编号，目录项..位于目录的第0块 */
...
/* 在inode编号为p_inode_nr的父目录中查找inode编号为c_inode_nr的子目录的名称，将其存入缓冲区path，成功则返回0，失败返回-1 */
static int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr, char* path, void* io_buf) {
	struct inode* parent_dir_inode = inode_open(cur_part, p_inode_nr);
	/* 填充all_blocks，将该目录的所有扇区地址存入all_blocks */
	uint8_t block_idx = 0;
...
/* 把当前工作目录绝对路径写入buf，size是buf大小，失败返回NULL */
char* sys_getcwd(char* buf, uint32_t size) {
	...
	memset(buf, 0, size);
	char full_path_reverse[MAX_PATH_LEN] = {0};    // 用于存绝对路径的缓冲区
	...
	/* 注意此时full_path_reverse中的路径是反的，即子目录在前，父目录在后，现在将路径反转 */
	char* last_slash;    // 用于记录字符串中最后一个'/'地址 */
	while ((last_slash = strrchr(full_path_reverse, '/'))) {
		uint16_t len = strlen(buf);
		strcpy(buf + len, last_slash);
		/* 修改full_path_reverse的结束字符，作为下一次执行strcpy中last_slash的边界 */
		*last_slash = 0;
	}
...
/* 将任务当前工作目录改为绝对路径path，成功则返回0，失败返回-1 */
