...
static int32_t copy_pcb_vaddrbitmap_stack0(struct task_struct* child_thread, struct task_struct* parent_thread) {
	/* 1. 复制父进程pcb所在的整个页，里面包含进程pcb相关信息，0特权级的栈、返回地址 */
	memcpy(child_thread, parent_thread, PG_SIZE);    // 先全部复制
	/* 下面再单独修改 */
	...
	/* 此时child_thread->userprog_vaddr.vaddr_bitmap.bits还指向父进程虚拟地址池的位图，修改使其指向自己的位图vaddr_btmp */
	memcpy(vaddr_btmp, child_thread->userprog_vaddr.vaddr_bitmap.bits, bitmap_pg_cnt * PG_SIZE);
	child_thread->userprog_vaddr.vaddr_bitmap.bits = vaddr_btmp;

	/* 调试使用 */
	ASSERT(strlen(child_thread->name) < 11);    // pcb.name最大长度限定为16
		...
		if (vaddr_btmp[idx_byte]) {
			...
			while (idx_bit < 8) {
				...
				if ((BITMAP_MASK << idx_bit) & vaddr_btmp[idx_byte]) {
					...
					/* 目的是切换到子进程的页目录表后，还能访问到父进程的数据 */
					memcpy(buf_page, (void*)prog_vaddr, PG_SIZE);

					/* 2. 切换到子进程的页表，避免下面申请内存的函数将pte及pde安装在父进程的页表中 */
...
static int32_t build_child_stack(struct task_struct* child_thread) {
	...
	/* 2. 为switch_to构建struct thread_struct，将其构建在紧临intr_stack之下的空间 */
	uint32_t* ret_addr_in_thread_stack = (uint32_t*)intr_0_stack - 1;

	/* 这三行不是必要的，只是为了疏理thread_stack中的关系 */
	...
	/* switch_to的返回地址更新为intr_exit，直接从中断中返回 */
	*ret_addr_in_thread_stack = (uint32_t)intr_exit;

	/* 下面的赋值只是为了构建的thread_stack更加清晰，其实不需要，因为在进入intr_exit后一系列pop会将寄存器中的内容覆盖 */
...
static int32_t copy_process(struct task_struct* child_thread, struct task_struct* parent_thread) {
	/* 创建内核缓冲区，作为父进程用户空间数据复制到子进程用户空间的中转 */
...
/* fork子进程，内核线程不可直接调用，只能通过用户进程进行系统调用进而调用 */
pid_t sys_fork(void) {
	...
	/* 添加到就绪线程队列和所有线程队列，子进程由schedule调度运行 */
