...
static void cmd_execute(uint32_t argc, char** argv) {
	...
	/* 如果是外部命令，fork出子进程并调用exec从磁盘上加载外部程序 */
		...
		if (pid) {
			int32_t status;
			/* 此时子进程若没有执行exit，my_shell进入TASK_WAITING状态，不会被调度，不再响应键入的命令 */
			int32_t child_pid = wait(&status);    // 管道命令非并行
			/* 程序正确不会执行 */
			if (child_pid == -1) {
...
void my_shell(void) {
	...
	while (1) {
		...
		/* 处理带管道命令 */
		/* 支持多重管道操作，cmd1|cmd2|...|cmdn，cmd1的标准输出和标准输入需要特殊处理 */
		if (pipe_symbol) {

			/* 1. 创建管道 */
			int32_t fd[2] = {-1};
			pipe(fd);                 // fd[0]用于输入，fd[1]用于输出

			fd_redirect(1, fd[1]);    // 将标准输出重定向到fd[1]，使输出信息重定向到内核缓冲区
