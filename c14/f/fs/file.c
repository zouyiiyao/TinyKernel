...
int32_t file_read(struct file* file, void* buf, uint32_t count) {
	...
	uint32_t* all_blocks = (uint32_t*)sys_malloc(48 + 512);    // 用来记录文件所有块地址
	if (all_blocks == NULL) {
	...
	uint32_t read_blocks = block_read_start_idx - block_read_end_idx;    // 如果增量为0，表示所有数据在同一扇区

	int32_t indirect_block_table;    // 用来获取一级间接块表地址
	uint32_t block_idx;              // 获取待读取的块地址

	/* 下面开始构建all_blocks块地址数据，用于存储用到第块地址 */
	if (read_blocks == 0) {    // 在同一扇区内读取数据
		...
		/* 情况二，待读取数据跨越直接块和间接块 */
			...
			/* 再写入间接块地址 */
			ASSERT(file->fd_inode->i_sectors[12] != 0);    // 确保已经分配一级间接块表
			indirect_block_table = file->fd_inode->i_sectors[12];
			ide_read(cur_part->my_disk, indirect_block_table, all_blocks + 12, 1);
		/* 情况三，只读取间接块 */
		} else {
	...
	/* 执行到这一步，所有要用到的块地址已经收集到all_blocks中，下面开始读数据 */
