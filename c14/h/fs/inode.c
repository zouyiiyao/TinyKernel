...
void inode_delete(struct partition* part, uint32_t inode_no, void* io_buf) {
	...
	char* inode_buf = (char*)io_buf;    // io_buf至少2扇区大小

	if (inode_pos.two_sec) {    // inode内容跨扇区
		/* 将原先硬盘上的内容先读出来 */
		ide_read(part->my_disk, inode_pos.sec_lba, inode_buf, 2);
		/* 将该inode内容清0 */
...
/* 回收inode对应文件的数据块以及inode自身资源（inode位图和inode表） */
void inode_release(struct partition* part, uint32_t inode_no) {
	...
	/* b. 如果一级间接块表存在，将128个间接块读入all_blocks，并释放一级间接块表所占的扇区 */
	...
	/* c. 所有数据块地址已经收集到all_blocks，下面逐个回收 */
	block_idx = 0;
	...
	/* 2. 回收文件占用的inode号（inode位图） */
	bitmap_set(&part->inode_bitmap, inode_no, 0);
	bitmap_sync(cur_part, inode_no, INODE_BITMAP);

	/* inode_delete仅用于调试，实际上不需要 */
